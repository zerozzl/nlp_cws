...
def train(dataset, dataloader, model, optimizer, lr_scheduler):
    model.train()
    loss_sum = 0
    for batch, data in enumerate(dataloader):
        ...
        loss = model(sents, masks, sents_len, decode=False, tags=tags)
        loss = loss.mean()
...
def evaluate(args, output_path, dataloader, tokenizer, model, epoch):
    ...
    with torch.no_grad():
        for batch, data in enumerate(dataloader):
            ...
            preds = model(sents, masks, sents_len)

            sents = [[s[0] for s in sent] for sent in sents]
...
def main(args):
    ...
    output_path += '_l%s' % args.num_hidden_layer
    if args.embed_freeze:
        output_path += '_embfix'
    if args.use_word_embed:
        output_path += '_wemb'
    if args.use_stack_lstm:
        output_path += '_stack'
    if args.use_crf:
        output_path += '_crf'
    ...
    word_to_id = {}
    if args.use_word_embed:
        word_to_id, pretrain_bigram_embed = load_pretrain_embedding(args.pretrained_word_emb_path,
                                                                    add_pad=True, add_unk=True)
    ...
    train_dataset = SIGHANDataset('%s/sighan2005-%s/train.txt' % (args.data_path, args.task), args.max_inp_size,
                                  do_pad=False, do_to_id=True, char_tokenizer=char_tokenizer,
                                  add_word_feature=args.use_word_embed, word_tokenizer=word_tokenizer,
                                  do_sort=True, debug=args.debug)
    ...
    if args.pretrained_model_path is not None:
        ...
        model = CWS(len(TAG_TO_ID), len(char_to_id), args.char_embed_size,
                    args.use_stack_lstm, args.hidden_size, args.num_hidden_layer,
                    args.embed_dropout_rate, args.lstm_dropout_rate,
                    args.use_crf, args.use_word_embed, len(word_to_id), args.word_embed_size,
                    args.embed_freeze, args.use_cpu)
        model = model.cpu() if args.use_cpu else model.cuda()

        model.init_char_embedding(np.array(pretrain_char_embed))
        if args.use_word_embed:
            model.init_word_embedding(np.array(pretrain_bigram_embed))
    ...
    while epoch < args.epoch_size:
        ...
        train_loss = train(train_dataset, train_dataloader, model, optimizer, lr_scheduler)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_path', dest='output_path',
                        default='../runtime/bilstm')
    parser.add_argument('--use_crf', dest='use_crf', type=bool,
                        default=False)
    parser.add_argument('--use_stack_lstm', dest='use_stack_lstm', type=bool,
                        default=False)
    ...
    parser.add_argument('--num_hidden_layer', dest='num_hidden_layer', type=int,
                        default=1)
    parser.add_argument('--embed_dropout_rate', dest='embed_dropout_rate', type=float,
                        default=0.6)
    parser.add_argument('--lstm_dropout_rate', dest='lstm_dropout_rate', type=float,
                        default=0.4)
